...
jest.mock("../components/Sidebar", () => ({
  Sidebar: ({ onClose, onCreateChat, onSwitchChat, openConfirmDeleteChatPopup }: SidebarProps) => (
    <div data-testid="sidebar">
      ...
      <button onClick={() => openConfirmDeleteChatPopup("session-1")}>Delete Chat</button>
    </div>
  ),
}));
...
describe("Chatbot component", () => {
  ...
  it("renders toggle button", () => {
    render(<Chatbot />);
    expect(screen.getByRole("button", { name: getChatbotText("toggleButtonLabel") })).toBeInTheDocument();
  });
  ...
  it("creates a new chat when clicking create button", async () => {
    render(<Chatbot />);
    fireEvent.click(screen.getByRole("button", { name: getChatbotText("toggleButtonLabel") }));
    fireEvent.click(screen.getByRole("button", { name: getChatbotText("createNewChat") }));
  ...
  it("sends a message and shows bot reply", async () => {
    sessionStorage.setItem(
      "chatbot-sessions",
      JSON.stringify([
        { id: "session-1", messages: [], createdAt: "2024-01-01", isLoading: false },
      ])
    );
  ...
  it("persists sessions on unmount", () => {
   ...
   it("logs error when createChatSession returns empty id", async () => {
    ...
    await waitFor(() => {
        expect(errorSpy).toHaveBeenCalledWith("Add error showage for a couple of seconds.");
    });
